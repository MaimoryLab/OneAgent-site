...
Upgrading OneAgent and upgrading an agent are two different things. This page is the first one.
...
No. An upgrade replaces the OneAgent application. It does not rewrite your agents' configuration files.
...
Agents that OneAgent installs are installed at their latest version, so updating one means installing the current latest again. It still [takes a backup](/en/help/05-backup/) before writing configuration.
...
That step disappears once signing is complete. Until then OneAgent will not document any way around your operating system's security policy.
