...
Alongside the command-line agents, BootAgent supports two desktop applications: **ChatGPT Desktop** and **WorkBuddy**. Both open from the Dock or Start menu and have no launch command.

## Install sources

Select the desktop agent in the environment overview and click install. BootAgent fetches whatever version the vendor currently serves:
...
Neither runs on Linux. BootAgent does not repackage or redistribute either one, so what you install is what the vendor published.

## Where the configuration lives

`ChatGPT Desktop` shares a configuration file with the Codex CLI: `~/.codex/config.toml`. Configure it once in BootAgent and both read the same result — there is no need to do it twice.
...
This is one real difference from command-line agents: BootAgent installs whatever the vendor's endpoint currently serves, so the environment overview shows no pinned version.
